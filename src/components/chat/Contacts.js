     ...
     (contact, idx) =>
      ...
      propTeamName(contact.teams) && (
       <div
        ...
        className={`b-shadow ${
         contact._id === activeContact._id && activeTeam.name === 'contacts'
       ...
       >
        <div className="noti">
         {contact.unreaded !== 0 && activeTeam.name === 'contacts' && (
          <span>{contact.unreaded > 9 ? '+9' : contact.unreaded}</span>
         )}
         ...
         {activeTeam.name === 'contacts' && (
          <p>
           {contact.messages[contact.messages.length - 1]?.message ||
            contact.messages[contact.messages.length - 1]?.file.name ||
            'No messages'}
          </p>
         )}
        </div>
